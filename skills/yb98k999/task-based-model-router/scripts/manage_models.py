#!/usr/bin/env python3
"""
OpenClaw Model Manager & Compute Router (v2.0.0)
...
class TaskPlanner:
    ...
    FALLBACK_TIERS = {
        "tier1": {"id": "anthropic/claude-opus-4.6", "price": 15.00, "role": "Architect/Reasoning"},
        "tier2": {"id": "openai/gpt-4o-mini",          "price": 0.60,  "role": "Coder/Drafter"},
        "tier3": {"id": "deepseek/deepseek-v3.2",      "price": 0.38,  "role": "Reviewer/Privacy"},
    }
    ...
    def _build_tiers_from_api(models):
        ...
        """
        priority_keywords = [
            "gpt-4o", "gpt-5", "claude-sonnet", "claude-opus", "claude-haiku",
            "gemini-3", "deepseek", "llama", "qwen3", "grok",
        ]
        ...
        prices = {
            "tier1": {"id": high["name"], "price": high["price"], "role": "Architect/Reasoning"},
            "tier2": {"id": mid["name"],  "price": mid["price"],  "role": "Coder/Drafter"},
            "tier3": {"id": low["name"],  "price": low["price"],  "role": "Reviewer/Privacy"},
        }
        model_map = {
            "tier1": high["name"],
            "tier2": mid["name"],
    ...
    def plan(self, task_description, execute=False):
        ...
        if category == "coding":
            ...
            steps = [
                {
                    "phase": "1. Research", "model_tier": "tier1", "reason": "Deep reasoning", "artifact": "RESEARCH.md",
                    "task": f"Conduct thorough research and analysis for: {task_description}. Save findings to 'RESEARCH.md'."
                },
                {
                    "phase": "2. Synthesize", "model_tier": "tier2", "reason": "Summarization", "artifact": "REPORT.md",
                    "task": "Read 'RESEARCH.md'. Synthesize the findings into a clear, structured report. Save to 'REPORT.md'."
                },
                {
                    "phase": "3. Fact-check", "model_tier": "tier3", "reason": "Verification", "artifact": "REVIEW.md",
            ...
            steps = [
                {
                    "phase": "1. Outline", "model_tier": "tier1", "reason": "Structure & strategy", "artifact": "OUTLINE.md",
                    "task": f"Create a detailed outline for: {task_description}. Save to 'OUTLINE.md'."
                },
                {
                    "phase": "2. Draft", "model_tier": "tier2", "reason": "Content generation", "artifact": "DRAFT.md",
                    "task": "Read 'OUTLINE.md'. Write the full draft following the outline. Save to 'DRAFT.md'."
                },
                {
                    "phase": "3. Polish", "model_tier": "tier3", "reason": "Proofreading", "artifact": "RESULT.md",
            ...
            steps = [
                {
                    "phase": "1. Ideate", "model_tier": "tier1", "reason": "Creative thinking", "artifact": "IDEAS.md",
                    "task": f"Brainstorm multiple creative approaches for: {task_description}. Save to 'IDEAS.md'."
                },
                {
                    "phase": "2. Execute", "model_tier": "tier2", "reason": "Production", "artifact": "RESULT.md",
            ...
            steps = [
                {
                    "phase": "1. Translate", "model_tier": "tier2", "reason": "Language conversion", "artifact": "TRANSLATION.md",
                    "task": f"Translate the following content accurately: {task_description}. Save to 'TRANSLATION.md'."
                },
                {
                    "phase": "2. Review", "model_tier": "tier3", "reason": "Quality check", "artifact": "RESULT.md",
            ...
            steps = [
                {
                    "phase": "1. Execute", "model_tier": "tier3", "reason": "Quick task", "artifact": "RESULT.md",
    ...
    def _display_plan(self, task, category, steps):
        ...
        savings_pct = ((cost_baseline - cost_optimized) / cost_baseline) * 100 if cost_baseline > 0 else 0
    ...
    def _execute_swarm(self, original_task, steps):
        ...
        for step in steps:
            plan_output["steps"].append({
                ...
                "system_prompt": self.prompts.get(
                    step.get("reason", "").lower().split()[0] if step.get("reason") else "",
                    {}
                ).get("system", ""),
        ...
        print(f"   Steps must run sequentially — each step depends on the previous artifact.")
...
def fetch_models():
    ...
    try:
        req = urllib.request.Request(
            TOKENROUTER_API,
            headers={'User-Agent': 'OpenClaw-ModelManager/2.0.0'}
        )
...
def filter_and_rank(models, limit=0):
    ...
    priority_keywords = [
        "gpt-4o", "gpt-5", "claude-sonnet", "claude-opus", "claude-haiku",
        "gemini-3", "deepseek", "llama", "qwen3", "grok", "minimax"
    ]
...
def display_models(models):
    ...
    for idx, m in enumerate(models, 1):
        ...
        print(f"| {idx} | `{model_name}` | ${in_price:.4f} | ${out_price:.4f} | {cache_str} | {mr} |")
...
def ensure_tokenrouter_provider(config):
    """Ensure TokenRouter provider is configured in the models.providers section."""
    providers = config.setdefault('models', {}).setdefault('providers', {})

    # Check if TokenRouter provider already exists
    tokenrouter_exists = False
    for provider_name, provider_config in providers.items():
        if provider_config.get('baseUrl') == 'https://open.palebluedot.ai/v1':
            tokenrouter_exists = True
            break

    # If not found, add TokenRouter provider
    if not tokenrouter_exists:
        providers['tokenrouter'] = {
            "baseUrl": "https://open.palebluedot.ai/v1",
            "apiKey": "<YOUR_TOKENROUTER_API_KEY>",
            "api": "openai-completions",
            "models": []
        }
        print("📝 [Config] Added TokenRouter provider to models.providers")
...
def update_tokenrouter_models(config, models):
    ...
    if tokenrouter_provider:
        ...
        print(f"📝 [Config] Updated TokenRouter provider with {len(model_names)} models")
...
def prepare_for_planning():
    """Prepare configuration before planning by ensuring TokenRouter setup."""
    print("🔄 [Setup] Preparing TokenRouter configuration...")

    # Load current config
    config = load_json_safe(CONFIG_FILE)

    # Ensure TokenRouter provider exists
    config = ensure_tokenrouter_provider(config)

    # Fetch current models from API
    models = fetch_models()
    if not models:
        print("⚠️ [Setup] Could not fetch models from API")
        return config  # Still return updated config even if models fetch failed

    # Update TokenRouter models
    update_tokenrouter_models(config, models)

    # Add all models to allow list
...
def main():
    ...
    if action == "plan":
        # Prepare configuration before planning
    ...
    if action == "enable":
        ...
        if target.isdigit():
            ...
            models = fetch_models()
            if not models:
                print("❌ Cannot fetch model list. Try enabling by name instead: enable <model_name>")
                return
    ...
    if action == "list":
        ...
        print(f"Error: Unknown action '{action}'. Use: list, enable, plan")
...
def enhanced_task_classification(task_description):
    ...
    patterns = {
        "coding": [
            "code", "program", "script", "debug", "function", "algorithm",
            "api", "database", "app", "test", "bug", "compile", "deploy", "refactor",
            "代码", "编程", "脚本", "程序", "调试", "测试", "开发", "编码",
            "接口", "部署", "重构", "修复", "函数", "算法", "数据库",
            "前端", "后端", "全栈", "爬虫", "框架", "模块",
        ],
        "writing": [
            "write", "article", "blog", "content", "story", "email", "letter",
            "essay", "copywriting", "documentation", "draft",
            "写作", "文章", "博客", "内容", "故事", "邮件", "信件",
            "文案", "文档", "稿件", "撰写", "起草",
        ],
        "analysis": [
            "analyze", "compare", "evaluate", "research", "study", "report",
            "data", "statistics", "insight", "metrics",
            "分析", "对比", "评估", "研究", "调研", "报告",
            "数据", "统计", "洞察", "指标", "复盘",
        ],
        "translation": [
            "translate", "convert language", "interpretation", "localize",
            "翻译", "转换语言", "本地化", "国际化", "多语言", "中译英", "英译中",
        ],
        "creative": [
            "creative", "brainstorm", "idea", "design", "artistic",
            "logo", "illustration", "prototype", "wireframe",
            "创意", "头脑风暴", "点子", "设计", "艺术",
            "原型", "线框图", "灵感", "构思",
        ],
        "simple": [
            "simple", "quick", "basic", "summarize", "list", "count",
            "lookup", "define", "explain",
            "简单", "快速", "基础", "总结", "列出", "计数",
            "查询", "定义", "解释", "概括",
        ],
    }
