...
def format_model_for_openclaw(model_id: str, with_provider_prefix: bool = True, append_free: bool = True) -> str:
    ...
    FIX (2026-04-01): OpenClaw needs the 'openrouter/' routing prefix for
    specific models (e.g. nvidia/nemotron:free → openrouter/nvidia/nemotron:free)
    but NOT for the built-in OpenRouter smart router 'openrouter/free'.
    """
    base_id = model_id

    # Handle openrouter/free special case: it's already a fully-qualified
    # router ID in the OpenRouter system. Never double-prefix it.
    ...
    # Always add openrouter/ routing prefix for specific models
