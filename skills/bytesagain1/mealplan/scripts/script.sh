...
_version() { echo "mealplan v2.0.0"; }

_help() {
    echo "Mealplan v2.0.0 — productivity toolkit"
    echo ""
    echo "Usage: mealplan <command> [args]"
    echo ""
    echo "Commands:"
    echo "  add                Add"
    echo "  plan               Plan"
...
_search() {
    local term="${1:?Usage: mealplan search <term>}"
...
case "${1:-help}" in
    add)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/add.log" 2>/dev/null || echo "  No entries yet. Use: mealplan add <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/add.log"
            local total=$(wc -l < "$DATA_DIR/add.log")
            echo "  [Mealplan] add: $input"
    ...
    plan)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/plan.log" 2>/dev/null || echo "  No entries yet. Use: mealplan plan <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/plan.log"
            local total=$(wc -l < "$DATA_DIR/plan.log")
            echo "  [Mealplan] plan: $input"
    ...
    track)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/track.log" 2>/dev/null || echo "  No entries yet. Use: mealplan track <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/track.log"
            local total=$(wc -l < "$DATA_DIR/track.log")
            echo "  [Mealplan] track: $input"
    ...
    review)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/review.log" 2>/dev/null || echo "  No entries yet. Use: mealplan review <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/review.log"
            local total=$(wc -l < "$DATA_DIR/review.log")
            echo "  [Mealplan] review: $input"
    ...
    streak)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/streak.log" 2>/dev/null || echo "  No entries yet. Use: mealplan streak <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/streak.log"
            local total=$(wc -l < "$DATA_DIR/streak.log")
            echo "  [Mealplan] streak: $input"
    ...
    remind)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/remind.log" 2>/dev/null || echo "  No entries yet. Use: mealplan remind <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/remind.log"
            local total=$(wc -l < "$DATA_DIR/remind.log")
            echo "  [Mealplan] remind: $input"
    ...
    prioritize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/prioritize.log" 2>/dev/null || echo "  No entries yet. Use: mealplan prioritize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/prioritize.log"
            local total=$(wc -l < "$DATA_DIR/prioritize.log")
            echo "  [Mealplan] prioritize: $input"
    ...
    archive)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/archive.log" 2>/dev/null || echo "  No entries yet. Use: mealplan archive <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/archive.log"
            local total=$(wc -l < "$DATA_DIR/archive.log")
            echo "  [Mealplan] archive: $input"
    ...
    tag)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tag.log" 2>/dev/null || echo "  No entries yet. Use: mealplan tag <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tag.log"
            local total=$(wc -l < "$DATA_DIR/tag.log")
            echo "  [Mealplan] tag: $input"
    ...
    timeline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/timeline.log" 2>/dev/null || echo "  No entries yet. Use: mealplan timeline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/timeline.log"
            local total=$(wc -l < "$DATA_DIR/timeline.log")
            echo "  [Mealplan] timeline: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: mealplan report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Mealplan] report: $input"
    ...
    weekly-review)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/weekly-review.log" 2>/dev/null || echo "  No entries yet. Use: mealplan weekly-review <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/weekly-review.log"
            local total=$(wc -l < "$DATA_DIR/weekly-review.log")
            echo "  [Mealplan] weekly-review: $input"
    ...
    *)
        ...
        echo "Run 'mealplan help' for available commands."
        exit 1
        ;;
esac