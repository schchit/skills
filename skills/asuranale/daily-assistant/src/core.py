...
def get_tasks_with_lines(file_path: Path, today: datetime = None) -> list[dict]:
    """解析文件中的所有任务，返回带行号的结构化数据（供 UI 使用）。"""
    today = today or datetime.now()
    content = file_path.read_text(encoding="utf-8")
    tasks = []
...
def toggle_task(file_path: Path, line_num: int) -> dict:
    """切换任务完成状态。返回 {success, completed, line}。"""
    content = file_path.read_text(encoding="utf-8")
    lines = content.split("\n")
    ...
    if stripped.startswith("- [ ] "):
        ...
        task_text = stripped[6:]
        task_text = task_text.replace(" 🔄", "").replace("🔄", "").rstrip()
        lines[line_num] = f"{indent}- [x] {task_text} ✅ {today_str}"
        completed = True
    elif stripped.startswith("- [x] "):
        task_text = stripped[6:]
        task_text = re.sub(r"\s*✅\s*\d{4}-\d{2}-\d{2}", "", task_text).rstrip()
        lines[line_num] = f"{indent}- [ ] {task_text}"
        completed = False
    else:
        return {"success": False, "error": "Not a task line"}

    file_path.write_text("\n".join(lines), encoding="utf-8")
    return {"success": True, "completed": completed, "line": line_num}
...
def add_task_to_file(
    ...
    parts = [f"- [ ] {description}"]
    if est_minutes:
        parts.append(f"⏱️{est_minutes}min")
    if deadline:
        parts.append(f"📅 {deadline}")
    priority_emoji = {"highest": " ⏫", "high": " 🔼", "low": " 🔽"}.get(priority, "")
    task_line = " ".join(parts) + priority_emoji
    ...
    for i, line in enumerate(lines):
        stripped = line.strip()
        if "📥" in stripped and ("任务" in stripped or "Task" in stripped):
            task_section_found = True
            insert_at = i + 1
        if task_section_found and (
            stripped.startswith("- [ ] ") or stripped.startswith("- [x] ")
        ):
            last_task_line = i

    if last_task_line is not None:
    ...
    file_path.write_text("\n".join(lines), encoding="utf-8")
    return {"success": True, "task": task_line, "line": insert_at}


def delete_task(file_path: Path, line_num: int) -> dict:
    """删除指定行的任务。"""
    content = file_path.read_text(encoding="utf-8")
    lines = content.split("\n")
