...
function query_scenery_api(params) {
  return new Promise((resolve, reject) => {
    const api_url = `${API_BASE_URL}${SCENERY_API_PATH}`;
...
async function main() {
  ...
  try {
    ...
    if (result.code === '0' || result.code === 0) {
      ...
      // 新结构：sceneryDataList 是数组，每个元素包含 pageDataList, sceneryList, desc
      // 旧结构：sceneryData 是单个对象
      const sceneryDataList = response_data?.sceneryDataList;
      const sceneryData = response_data?.sceneryData;
      
      if (Array.isArray(sceneryDataList) && sceneryDataList.length > 0) {
        // 新结构：遍历所有列表，每个列表都有 desc 说明
        let hasOutput = false;
        sceneryDataList.forEach((item, index) => {
          if (item.sceneryList && item.sceneryList.length > 0) {
            // 输出列表说明（desc）
            if (item.desc) {
              console.log(`📌 ${item.desc}\n`);
            } else if (sceneryDataList.length > 1) {
        ...
        if (!hasOutput) {
          console.log('⚠️ 无结果');
          console.log('未找到符合条件的景区，请尝试调整查询条件。');
        }
      } else if (sceneryData) {
        // 旧结构：单个对象
        console.log(format_scenery_result(sceneryData, use_table, use_plain_link));
      } else {
        console.log('⚠️ 无结果');
        console.log('未找到符合条件的景区，请尝试调整查询条件。');
      }
