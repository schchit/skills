...
function query_bus_api(params) {
  return new Promise((resolve, reject) => {
    const api_url = `${API_BASE_URL}${BUS_API_PATH}`;
...
/**
 * 格式化长途汽车结果
 * @param {object} bus_data - 长途汽车数据（包含 pageDataList）
...
function format_bus_result(bus_data, use_table = false, use_plain_link = false) {
  ...
  if (use_table) {
    // 表格格式（只显示出发地、目的地和预订链接）
    output += '| 出发地 | 目的地 | PC 预订 | 手机预订 |\n';
    output += '|--------|--------|--------|---------|\n';
    ...
    buses.forEach((bus) => {
      ...
      output += `| ${dep_station} | ${arr_station} | ${pc_link_md} | ${mobile_link_md} |\n`;
    });
  } else {
    // 卡片格式
    buses.forEach((bus) => {
      // 汽车票字段：depName, arrName 等
      const bus_no = bus.busNo || bus.busName || '-';
      const dep_station = bus.depName || bus.depStationName || '-';
      const arr_station = bus.arrName || bus.arrStationName || '-';
      ...
      let ticket_info = '';
      if (bus.ticketList && bus.ticketList.length > 0) {
        const available_tickets = bus.ticketList.filter(t => parseInt(t.ticketLeft) > 0);
        if (available_tickets.length > 0) {
          ticket_info = available_tickets.map(t => `${t.ticketType}(${t.ticketLeft})`).join(', ');
        } else {
          ticket_info = '售罄';
        }
      } else if (bus.seatLeft) {
        ticket_info = `余座 (${bus.seatLeft})`;
      }
      
      // 汽车票链接字段（优先使用标准字段，补偿逻辑与其他资源一致）
      const pc_link = bus.pcRedirectUrl || bus.clawRedirectUrl || bus.redirectUrl || '#';
      const mobile_link = bus.clawRedirectUrl || bus.redirectUrl || '#';
      ...
      output += `### 🚌 ${bus_no} | ${dep_station} → ${arr_station}\n`;
...
async function main() {
  ...
  try {
    ...
    if (result.code === '0' || result.code === 0) {
      ...
      // 汽车票结构：busDataList 是数组，每个元素包含 pageDataList（实际班次数据）
      // 注意：汽车票使用 pageDataList 而不是 busList
      const busDataList = response_data?.busDataList;
      const busData = response_data?.busData;
      
      if (Array.isArray(busDataList) && busDataList.length > 0) {
        // 新结构：遍历所有列表，从 pageDataList 获取班次数据
        let hasOutput = false;
        busDataList.forEach((item, index) => {
          // 汽车票数据在 pageDataList 中
          if (item.pageDataList && item.pageDataList.length > 0) {
            // 输出列表说明（desc）
            if (item.desc) {
              console.log(`📌 ${item.desc}\n`);
            } else if (busDataList.length > 1) {
              console.log(`📌 列表 ${index + 1}\n`);
            }
            // 传入 pageDataList 作为数据源
            console.log(format_bus_result({ pageDataList: item.pageDataList }, use_table, use_plain_link));
            hasOutput = true;
          }
        });
        if (!hasOutput) {
          console.log('⚠️ 无结果');
          console.log('未找到符合条件的长途汽车，请尝试调整查询条件。');
        }
      } else if (busData) {
        // 旧结构：单个对象
        console.log(format_bus_result(busData, use_table, use_plain_link));
      } else {
        console.log('⚠️ 无结果');
        console.log('未找到符合条件的长途汽车，请尝试调整查询条件。');
      }
