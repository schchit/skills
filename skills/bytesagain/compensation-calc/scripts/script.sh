#!/usr/bin/env bash
# compensation-calc - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${COMPENSATION_CALC_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/compensation-calc}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
compensation-calc v$VERSION

Multi-purpose utility tool

Usage: compensation-calc <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "compensation-calc v$VERSION" ;;
