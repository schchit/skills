#!/usr/bin/env bash
# ab-test-calculator - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${AB_TEST_CALCULATOR_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/ab-test-calculator}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
ab-test-calculator v$VERSION

Multi-purpose utility tool

Usage: ab-test-calculator <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "ab-test-calculator v$VERSION" ;;
