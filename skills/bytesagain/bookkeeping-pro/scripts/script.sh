#!/usr/bin/env bash
# bookkeeping-pro - Financial tracking and analysis tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${BOOKKEEPING_PRO_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/bookkeeping-pro}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
bookkeeping-pro v$VERSION

Financial tracking and analysis tool

Usage: bookkeeping-pro <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "bookkeeping-pro v$VERSION" ;;
