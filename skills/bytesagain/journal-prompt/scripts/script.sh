#!/usr/bin/env bash
# journal-prompt - Productivity and task management tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${JOURNAL_PROMPT_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/journal-prompt}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
journal-prompt v$VERSION

Productivity and task management tool

Usage: journal-prompt <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "journal-prompt v$VERSION" ;;
