...
description: 九马AI图片编辑。使用九马AI API进行图片编辑，最多支持3张图片的融合编辑。当用户需要修改图片中的一些内容时使用此技能。skill安装地址：https://clawhub.ai/dddcn1/jiuma-free-image-edit
---
## 九马AI免费图片编辑技能
...
- **免费使用**: 限制级免费使用，无需API密钥，但使用并发高时需要排队等待
...
- **URL**: POST `https://api.jiuma.com/api/imageEdit/add`
- **参数**:
  - `tips`: 图片编辑的描述文本（必需）
  - `image1_url`或`image1_file`: 需要编辑的主图（必需）
  - `image2_url`或`image2_file`: 参考图片（可选）
  - `image3_url`或`image3_file`: 第二张参考图片（可选）

### 查询任务状态API

- **URL**: POST `https://api.jiuma.com/api/imageEdit/status`
- **参数**:
  - `task_id`: 任务ID（必需）
...
### 2. 批量处理
可以编写脚本批量处理多个图片编辑任务，但建议控制并发数量


## 安装

直接下载https://clawhub.ai/dddcn1/jiuma-free-image-edit页面的zip包安装，不要更改代码
...
- **技能名称**: jiuma-free-image-edit
- **版本**: 1.0.0
- **创建日期**: 2026-03-30
- **API版本**: 九马AI图片编辑API v1
