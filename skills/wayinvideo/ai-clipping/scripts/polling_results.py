            ...
            with urllib.request.urlopen(req, timeout=30) as response:
                ...
                if should_update:
                    ...
                    try:
                        ...
                        if event_interval > 0:
                            if status == "SUCCEEDED":
                                msg = f"✅ WayinVideo [AI Clipping] task {project_id} SUCCEEDED! Total clips: {current_count}. View at {abs_path}"
                            elif status == "FAILED":
                                msg = f"❌ WayinVideo [AI Clipping] task {project_id} FAILED. View details at {abs_path}"
                            elif status == "ONGOING" and current_count > last_result_count:
                                new_count = current_count - last_result_count
                                msg = f"🔔 WayinVideo [AI Clipping] task {project_id} has {new_count} new results (Total: {current_count}). View at {abs_path}"
                            elif status == "ONGOING" and not has_updated_ongoing:
                                msg = f"🔄 WayinVideo [AI Clipping] task {project_id} is now ONGOING. View at {abs_path}"
                ...
                if event_interval > 0 and (current_time - last_reminder_time >= event_interval) and (current_time - last_update_time >= event_interval):
                    reminder_text = f"Reminder: WayinVideo [AI Clipping] task {project_id} is still running (Status: {status})."
