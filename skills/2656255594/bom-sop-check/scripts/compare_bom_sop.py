...
def normalize_for_compare(text: str) -> str:
    ...
    # 英文标点：,.!?;:""''[]()-...<>
    # 分隔符：:：|｜/\\、
    punctuation_chars = r'[，。！？；：""''【】（）、—…《》,.!?;:"\'\[\]\(\)\-…<>:：|｜/\\、\s\n\r\t]+'
    text = re.sub(punctuation_chars, '', text)
...
def normalize_position_for_compare(text: str) -> str:
    ...
    for pos in positions:
        pos = pos.strip().upper()  # 统一转大写
        if pos:
            normalized_positions.append(pos)
    
    # 去重后排序
    unique_positions = sorted(set(normalized_positions))
...
def merge_bom_items(bom_items_list: List[Dict[str, Dict]]) -> Dict[str, Dict]:
    """合并多个 BOM 物料列表
    ...
    for bom_items in bom_items_list:
        for code, item in bom_items.items():
            if code not in merged:
                ...
                try:
                    ...
                    new_qty = float(item.get('qty', 0))
                    existing['qty'] = str(existing_qty + new_qty)
                except:
                    pass
                
                # 合并位号
                existing_pos = existing.get('remark', '')
                new_pos = item.get('remark', '')
                if existing_pos and new_pos:
                    # 合并位号，用逗号分隔
                    if new_pos not in existing_pos:
                        existing['remark'] = f"{existing_pos},{new_pos}"
                elif new_pos:
                    existing['remark'] = new_pos
...
def parse_bom_file(bom_path: str) -> Dict[str, Dict]:
    ...
    for row_match in re.finditer(r'<row r="(\d+)"[^>]*>(.*?)</row>', sheet_content, re.DOTALL):
        ...
        if code in items:
            # 累加数量
            try:
                old_qty = float(items[code]['qty'])
                new_qty = old_qty + qty
                items[code]['qty'] = str(int(new_qty) if new_qty == int(new_qty) else round(new_qty, 2))
            except:
                pass
            # 合并位号
            old_remark = items[code].get('remark', '')
            new_remark = row_data.get(remark_col, '')
            if old_remark and new_remark and new_remark not in old_remark:
                items[code]['remark'] = f"{old_remark},{new_remark}"
            elif new_remark and not old_remark:
                items[code]['remark'] = new_remark
        else:
            items[code] = {
                'code': code,
                'name': name,
...
def parse_sop_file(sop_path: str) -> Dict:
    ...
    for sheet_file in sorted(os.listdir(worksheets_dir)):
        ...
        try:
            with open(candidate, 'r', encoding='utf-8') as f:
                content = f.read()
            count = len(re.findall(r'<c r="AQ\d+"', content))
            if count > best_count:
                best_count = count
    ...
    if best_sheet:
        sheet_path = best_sheet
        logger.debug(f"选择工作表: {os.path.basename(sheet_path)} (AQ列单元格数: {best_count})")
    ...
    for row_match in re.finditer(r'<row r="(\d+)"[^>]*>(.*?)</row>', sheet_content, re.DOTALL):
        ...
        if code in result['items']:
            old_item = result['items'][code]
            # 累加数量
            try:
                old_qty = float(old_item.get('qty', 0))
                new_qty = float(item_data['qty']) if item_data['qty'] else 0
                total_qty = old_qty + new_qty
                old_item['qty'] = str(int(total_qty) if total_qty == int(total_qty) else round(total_qty, 2))
            except:
                pass
            # 合并位号
            old_pos = old_item.get('position', '')
            new_pos = item_data['position']
            if old_pos and new_pos and new_pos not in old_pos:
                old_item['position'] = f"{old_pos},{new_pos}"
            elif new_pos and not old_pos:
                old_item['position'] = new_pos
        else:
            result['items'][code] = item_data
...
def compare_bom_sop(bom_items: Dict, sop_items: Dict) -> Dict:
    ...
    for code in bom_codes & sop_codes:
        ...
        if bom_name and sop_name:
            # 使用规范化比较（忽略标点符号等）
            bom_name_norm = normalize_for_compare(bom_name)
            sop_name_norm = normalize_for_compare(sop_name)
            if bom_name_norm != sop_name_norm:
                diff_types.append('名称规格')
                diff_details['名称规格'] = {'bom': bom_name, 'sop': sop_name}
        
        # 对比位号（分割后排序比较，忽略顺序和括号内容）
        bom_pos = safe_str(bom_item.get('remark', ''), 100)
        sop_pos = safe_str(sop_item.get('position', ''), 100)
        if bom_pos and sop_pos:
            # 使用位号专用规范化比较（分割后排序）
            bom_pos_norm = normalize_position_for_compare(bom_pos)
            sop_pos_norm = normalize_position_for_compare(sop_pos)
            if bom_pos_norm != sop_pos_norm:
...
def generate_marked_file(result: Dict, bom_items: Dict, sop_data: Dict, output_dir: str) -> Optional[str]:
    ...
    # 读取 SOP 工作表（使用 parse_sop_file 中已选择的工作表）
    sheet_path = sop_data.get('sheet_path')
    logger.debug(f"sop_data['sheet_path'] = {sheet_path}")
    if not sheet_path or not os.path.exists(sheet_path):
        ...
        if best_sheet:
            sheet_path = best_sheet
            logger.debug(f"重新选择工作表: {sheet_path} (AQ列单元格数: {best_count})")
    ...
    if cellxfs_match:
        ...
        # 处理填充样式 - 检查是否已有红色/黄色填充
        fills_match = re.search(r'<fills count="(\d+)">(.*?)</fills>', styles_content, re.DOTALL)
        original_fills_count = 0
        red_fill_id = -1
        yellow_fill_id = -1
        
        if fills_match:
            original_fills_count = int(fills_match.group(1))
            fills_content = fills_match.group(2)
            
            # 查找已有的红色和黄色填充
            for i, fill_match in enumerate(re.finditer(r'<fill>(.*?)</fill>', fills_content, re.DOTALL)):
                fill_text = fill_match.group(1)
                if 'FFFF0000' in fill_text and 'patternType="solid"' in fill_text:
                    red_fill_id = i
                    logger.debug(f"找到已有红色填充: Fill {i}")
                elif 'FFFFFF00' in fill_text and 'patternType="solid"' in fill_text:
                    yellow_fill_id = i
                    logger.debug(f"找到已有黄色填充: Fill {i}")
            
            # 添加缺失的填充
            header_fill = '<fill><patternFill patternType="solid"><fgColor rgb="FFD9E8F7"/><bgColor indexed="64"/></patternFill></fill>'
            new_fills = fills_content
            fills_to_add = 0
            
            if red_fill_id < 0:
                new_fills += RED_FILL
                red_fill_id = original_fills_count + fills_to_add
                fills_to_add += 1
                logger.info(f"添加新红色填充: Fill {red_fill_id}")
            if yellow_fill_id < 0:
                new_fills += YELLOW_FILL
                yellow_fill_id = original_fills_count + fills_to_add
                fills_to_add += 1
                logger.info(f"添加新黄色填充: Fill {yellow_fill_id}")
            
            # 添加浅蓝色填充（表头用）
            new_fills += header_fill
            header_fill_id = original_fills_count + fills_to_add
            fills_to_add += 1
            
            styles_content = styles_content.replace(
                fills_match.group(0),
                f'<fills count="{original_fills_count + fills_to_add}">{new_fills}</fills>'
            )
        ...
        if fonts_match:
            ...
            bold_font_id = fonts_count
            styles_content = styles_content.replace(
                fonts_match.group(0),
                f'<fonts count="{fonts_count + 1}">{fonts_content}{bold_font}</fonts>'
            )
        
        # 计算样式索引（使用实际的 xf 标签数量，而不是 count 属性）
        actual_xfs_count = len(re.findall(r'<xf[^>]*/?\s*>', xfs_content))
        red_xf_idx = actual_xfs_count
        yellow_xf_idx = actual_xfs_count + 1
        border_xf_idx = actual_xfs_count + 2  # 带边框样式索引
        header_xf_idx = actual_xfs_count + 3  # 表头样式索引
        
        logger.debug(f"cellXfs count 属性: {xfs_count}, 实际 xf 标签数: {actual_xfs_count}")
        logger.debug(f"计算样式索引: 红={red_xf_idx}, 黄={yellow_xf_idx}, 边框={border_xf_idx}, 表头={header_xf_idx}")
        
        # 添加新的 cellXfs
        new_xfs_content = xfs_content
        # 红色填充样式
        new_xfs_content += f'<xf numFmtId="0" fontId="0" fillId="{red_fill_id}" borderId="0" xfId="0"/>'
        # 黄色填充样式
        new_xfs_content += f'<xf numFmtId="0" fontId="0" fillId="{yellow_fill_id}" borderId="0" xfId="0"/>'
        # 边框样式（无填充）
        new_xfs_content += f'<xf numFmtId="0" fontId="0" fillId="0" borderId="{new_border_id}" xfId="0"/>'
        # 表头样式（加粗 + 浅蓝背景 + 边框）
        new_xfs_content += f'<xf numFmtId="0" fontId="{bold_font_id}" fillId="{header_fill_id}" borderId="{new_border_id}" xfId="0"/>'
        ...
        logger.info(f"样式索引: 红色={red_xf_idx}(fillId={red_fill_id}), 黄色={yellow_xf_idx}(fillId={yellow_fill_id})")
        logger.debug(f"cellXfs 添加: fillId {red_fill_id} 和 {yellow_fill_id}")
    ...
    verify_xfs_match = re.search(r'<cellXfs count="(\d+)">(.*?)</cellXfs>', verify_content, re.DOTALL)
    if verify_xfs_match:
        verify_xfs = re.findall(r'<xf[^/]*/>', verify_xfs_match.group(2))
        if len(verify_xfs) > red_xf_idx:
            logger.debug(f"验证: 样式 {red_xf_idx} = {verify_xfs[red_xf_idx]}")
            ...
            for cells_info in sop_data['cells'][code]:
                ...
                for diff_type in diff['diff_types']:
                    ...
                    if col:
                        ...
                        idx = red_xf_idx
                        # 处理非自闭合标签: <c r="XX" ...>
                        pattern1 = rf'<c r="{cell_ref}"((?!/>)[^>])*?>'
                        match1 = re.search(pattern1, sheet_content)
                        if match1:
                            tag = match1.group(0)
                            if ' s="' in tag:
                                new_tag = re.sub(r' s="\d+"', f' s="{idx}"', tag)
                            else:
                                new_tag = tag.replace('>', f' s="{idx}">')
                            sheet_content = sheet_content.replace(tag, new_tag, 1)
                        # 处理自闭合标签: <c r="XX" .../>
                        else:
                            pattern2 = rf'<c r="{cell_ref}"[^>]*/>'
                            match2 = re.search(pattern2, sheet_content)
                            if match2:
                                tag = match2.group(0)
                                if ' s="' in tag:
                                    new_tag = re.sub(r' s="\d+"', f' s="{idx}"', tag)
    ...
    # ============ 重新读取样式以获取最终索引 ============
    # create_report_sheet 可能会修改样式，需要重新获取索引
    with open(styles_path, 'r', encoding='utf-8') as f:
        final_styles = f.read()
    
    xfs_match = re.search(r'<cellXfs count="(\d+)">(.*?)</cellXfs>', final_styles, re.DOTALL)
    if xfs_match:
        final_count = int(xfs_match.group(1))
        final_xfs_content = xfs_match.group(2)
        final_all_xfs = re.findall(r'<xf[^/]*/>', final_xfs_content)
        
        logger.debug(f"最终 styles.xml: cellXfs count={final_count}, 实际 xf 标签数={len(final_all_xfs)}")
        
        # 查找红色和黄色样式的实际索引
        red_xf_idx = None
        yellow_xf_idx = None
        for i, xf in enumerate(final_all_xfs):
            fill_match = re.search(r'fillId="(\d+)"', xf)
            if fill_match:
                fill_id = int(fill_match.group(1))
                if fill_id == red_fill_id and red_xf_idx is None:
                    red_xf_idx = i
                elif fill_id == yellow_fill_id and yellow_xf_idx is None:
                    yellow_xf_idx = i
        
        if red_xf_idx is None or yellow_xf_idx is None:
            # 如果没找到，使用最后的样式
            red_xf_idx = len(final_all_xfs) - 4
            yellow_xf_idx = len(final_all_xfs) - 3
        
        logger.info(f"最终样式索引: 红色={red_xf_idx}(fillId={red_fill_id}), 黄色={yellow_xf_idx}(fillId={yellow_fill_id})")
        
        # 查找边框样式（不带 quotePrefix，fillId=0）
        # 我们添加的边框样式应该在倒数第2个位置（红色、黄色、边框、表头）
        border_xf_idx = len(final_all_xfs) - 2  # 边框样式在倒数第2个
        
        # 验证这个样式不带 quotePrefix
        if border_xf_idx < len(final_all_xfs):
            border_xf = final_all_xfs[border_xf_idx]
            if 'quotePrefix' in border_xf:
                # 如果有 quotePrefix，找一个不带 quotePrefix 的边框样式
                for i, xf in enumerate(final_all_xfs):
                    border_match = re.search(r'borderId="(\d+)"', xf)
                    fill_match = re.search(r'fillId="(\d+)"', xf)
                    if border_match and fill_match:
                        fill_id = int(fill_match.group(1))
                        if fill_id == 0 and 'quotePrefix' not in xf:
                            border_xf_idx = i
                            logger.debug(f"找到不带 quotePrefix 的边框样式: {i}")
                            break
        
        logger.debug(f"边框样式索引: {border_xf_idx}")
    
    # 重新读取工作表并使用最终样式索引进行标注
    with open(sheet_path, 'r', encoding='utf-8') as f:
        sheet_content = f.read()
    
    logger.debug(f"重新读取工作表，长度: {len(sheet_content)}, 包含 AX155: {'AX155' in sheet_content}")
    logger.debug(f"工作表前500字符: {sheet_content[:500]}")
    
    # 重新标注差异单元格（使用最终的样式索引）
    marked_cells = []
    logger.debug(f"差异物料数: {len(result['diffs'])}, SOP独有数: {len(result['only_sop'])}")
    for diff in result['diffs']:
        code = diff['code']
        if code in sop_data['cells']:
            for cells_info in sop_data['cells'][code]:
                row = cells_info['row']
                logger.debug(f"物料 {code} 行 {row}, cells_info: name_col={cells_info.get('name_col')}, position_col={cells_info.get('position_col')}, qty_col={cells_info.get('qty_col')}")
                for diff_type in diff['diff_types']:
                    if diff_type == '名称规格':
                        col = cells_info.get('name_col', 'AX')
                    elif diff_type == '位号':
                    ...
                    if col:
                        ...
                        pattern1 = rf'<c r="{cell_ref}"((?!/>)[^>])*?>'
                        match1 = re.search(pattern1, sheet_content)
                        logger.debug(f"  尝试匹配 {cell_ref}: match1={match1 is not None}")
                        if match1:
                            tag = match1.group(0)
                            if ' s="' in tag:
                                new_tag = re.sub(r' s="\d+"', f' s="{idx}"', tag)
    ...
    logger.info(f"最终标注: 红色={red_xf_idx}, 黄色={yellow_xf_idx}, 标注了 {len(marked_cells)} 个单元格")
...
def create_report_sheet(result: Dict, bom_items: Dict, sop_data: Dict, border_xf_idx: int, header_xf_idx: int) -> None:
    ...
    # ============ 表格3: 数量差异明细 ============
    title3_idx = add_string('【数量差异明细】（BOM与SOP数量不一致）')
    rows_xml += f'<row r="{row_num}"><c r="A{row_num}" t="s"><v>{title3_idx}</v></c></row>'
    row_num += 1
    
    # 表头
    qty_headers = ['物料编码', '物料名称', 'BOM数量', 'SOP数量', '差异数量']
    header_cells = ''
    for col_idx, header in enumerate(qty_headers):
        col_letter = chr(ord('A') + col_idx)
        str_idx = add_string(header)
        header_cells += f'<c r="{col_letter}{row_num}" s="{header_xf_idx}" t="s"><v>{str_idx}</v></c>'
    rows_xml += f'<row r="{row_num}">{header_cells}</row>'
    ...
    qty_diff_count = 0
    for diff in result['diffs']:
        if '数量' in diff['diff_types']:
            code = diff['code']
            name = diff.get('name', '')
            details = diff.get('details', {}).get('数量', {})
            bom_qty = details.get('bom', '')
            sop_qty = details.get('sop', '')
            
            # 计算差异数量
            try:
                bom_val = float(bom_qty) if bom_qty else 0
                sop_val = float(sop_qty) if sop_qty else 0
                diff_val = bom_val - sop_val
                if diff_val == int(diff_val):
                    diff_str = str(int(diff_val))
                else:
            ...
            code_idx = add_string(code)
            name_idx = add_string(safe_str(name, 30))
            bom_qty_idx = add_string(safe_str(bom_qty, 20))
            sop_qty_idx = add_string(safe_str(sop_qty, 20))
            diff_idx = add_string(diff_str)
            
            cells = f'<c r="A{row_num}" s="{border_xf_idx}" t="s"><v>{code_idx}</v></c>'
            cells += f'<c r="B{row_num}" s="{border_xf_idx}" t="s"><v>{name_idx}</v></c>'
            cells += f'<c r="C{row_num}" s="{border_xf_idx}" t="s"><v>{bom_qty_idx}</v></c>'
            cells += f'<c r="D{row_num}" s="{border_xf_idx}" t="s"><v>{sop_qty_idx}</v></c>'
            cells += f'<c r="E{row_num}" s="{border_xf_idx}" t="s"><v>{diff_idx}</v></c>'
            rows_xml += f'<row r="{row_num}">{cells}</row>'
            row_num += 1
            qty_diff_count += 1
...
<pageSetup paperSize="9" orientation="portrait"/>
    ...
    logger.info(f"创建校对报告工作表: SOP独有 {sop_only_count} 项, BOM独有 {bom_only_count} 项, 数量差异 {qty_diff_count} 项")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='BOM 与 SOP 校对工具 v2.1.0')
    parser.add_argument('bom', nargs='+', help='BOM 文件路径（支持多个文件，会自动合并）')
    ...
    try:
        # 解析并合并 BOM 文件
        if len(bom_paths) == 1:
            print(f"解析 BOM 文件: {os.path.basename(bom_paths[0])}")
            bom_items = parse_bom_file(bom_paths[0])
        else:
            print(f"解析并合并 {len(bom_paths)} 个 BOM 文件:")
            bom_items_list = []
            for bom_path in bom_paths:
                print(f"  - {os.path.basename(bom_path)}")
                items = parse_bom_file(bom_path)
                bom_items_list.append(items)
            bom_items = merge_bom_items(bom_items_list)
