#!/usr/bin/env bash
# blast — Blast reference tool. Use when working with blast in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
blast v$VERSION — Blast Reference Tool
...
## What is Blast?
Blast (blast) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with blast.

## Key Concepts
- Core blast principles and fundamentals
- How blast fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Blast Matters
Understanding blast is critical for:
...
## Getting Started
1. Understand the basic blast concepts
...
cmd_formulas() {
    cat << 'EOF'
# Blast — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Blast — Instruments & Tools Overview

## Primary Instruments
- Core tools used in blast operations
...
cmd_glossary() {
    cat << 'EOF'
# Blast — Key Terms & Definitions

## Core Terminology
- **Blast**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "blast v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: blast help"; exit 1 ;;
esac
